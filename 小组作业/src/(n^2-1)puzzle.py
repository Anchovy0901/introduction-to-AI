...
import time
#初始状态
print("请输入数码的级数：   (可选项：3、4）")
n = int(input())
print("请输入初始状态，以",n,"*",n,"的形式，数字以空格相隔，行末以回车结尾")
stat = [[0]*n]*n
for i in range(n):
    stat[i] = input().split(" ")
    stat[i] = [int(j) for j in stat[i]]
print("起始状态存储为：")
for i in range(n):
    print(stat[i])
print()
print("请输入目标状态，以",n,"*",n,"的形式，数字以空格相隔，行末以回车结尾")
target = [[0]*n]*n
for i in range(n):
    target[i] = input().split(" ")
    target[i] = [int(j) for j in target[i]]
print("目标状态存储为：")
for i in range(n):
    print(target[i])
print()
...
class puzzle:
    ...
    def see(self):
        for i in range(n):
            print(self.stat[i])
        print("总代价F为",self.F,"当前深度G为",self.G,"估计代价H为",self.H)
        print()
    #查看找到的解是如何从头移动的
    def seeAns(self):
        ans=[]
...
def Astar(startStat):
    ...
    while(open):
        ...
        if(minFStat.H==0):
            print("已经找到最优解！")
            print("搜索的次数：",time,"移动总步数：",minFStat.G)
            print()
            minFStat.seeAns()
            break
...
print("运行花费的时间: ",(int)(elapsed*1000*1000)," 微秒(μs)")